...
int main() {
  ...
  while (file >> utmp)
  {
    SI_Data.push_back(stoi(utmp, 0, 10));
  }
  file.close();

  ofstream Re_file;
  //清空后写入多重并集文件
  Re_file.open("./data/Multi_Inter_Data.csv", ios::trunc);
  ...
  for(int j = 0; j < GroupNum; ++j){
    ...
    h = MulMod(hs, ht, N*N);
    //cout << "h:" << h << endl;
    //对每组数据中的每一个数字进行编码求较小值
    for(int i = 0; i < eleNum; ++i) {
      vector<int> code1(maxNum, 0);
      vector<int> code2(maxNum, 0);
      int Sindex, Tindex; //双方隐私输入在全集U中的位置（下标）
      //编码
      Encode_LT(S_Freq[j][i], U, Sindex, code1);
      Encode_LT(T_Freq[j][i], U, Tindex, code2);
