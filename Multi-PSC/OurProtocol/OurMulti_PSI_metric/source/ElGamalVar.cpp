...
void ElGamalVar::addCode(ElGamal_Cipher *Pair, vector<int> U, NTL::ZZ Splaintext, ElGamal_Cipher &AC_result, int &index) {
    //Steven首先要得到解密得到的明文所在位置
    int num = U.size();
    int i = 0;
    while( U[i] != Splaintext && i < num) {
        ++i;
    }
    index = i;
    //cout << "密文所在位置：" << i;
    ...
    result.fir = Pair[i].fir;
    result.sec = Pair[i].sec;
    //cout << "result:(" << result.fir << "," << result.sec << endl;
    for(--i; i >= 0; --i) {
        //cout << "i:" << i << endl;
        plainAdd(result, Pair[i], temp);
        result = temp;
    }
