...
int main()
{
  ...
  file.close();
  file.open("./data/Standard_Inter_Data.csv", ios::in);
  vector<int> SI_Data;
  while (file >> tmp)
  {
    SI_Data.push_back(stoi(tmp, 0, 10));
  }
  file.close();
  ...
  int eleNum = SI_Data.size();  //交集中元素个数
  //多重交集元素重数数组CW
  NTL::ZZ *CW = new ZZ[eleNum];
  ...
  for(int i = 0; i < eleNum; ++i) {
    ...
    //编码
    Encode_LT(S_Freq[i], U, Sindex, code1);
    Encode_LT(T_Freq[i], U, Tindex, code2);
    
    //双方对编码进行加密，同时对参与方所拥有的数据进行了赋值
    ElGamal_Cipher *Spair = new ElGamal_Cipher[number];
    ElGamal_Cipher *Tpair = new ElGamal_Cipher[number];
    Steven.Encrypt(number, code1, Spair, h, S_Freq[i]);
    Tom.Encrypt(number, code2, Tpair, h, T_Freq[i]);
    ...
    Steven.addCode(Tpair, U, Splaintext, AC1_result, Sindex);
    ...
    Tom.addCode(Spair, U, Tplaintext, AC2_result, Tindex);
  ...
  for(int i = 0; i < eleNum; ++i) {
    for(ZZ j; j < CW[i]; ++j) {
      Mul_I.push_back(SI_Data[i]);
    }
  }
  //将结果保存至文件中
  file.open("./data/Multi_Inter_Data.csv", ios::out);
