...
int main()
{
  ...
  cin >> input2;
  int length = U.size();
  Encode_GT(input1, U, Sindex, code1);
  Encode_GT(input2, U, Tindex, code2);
  ...
  //cout << "Steven 明文：" << Splaintext << endl;
  Tom.Decrypt(Tplaintext, ct.sec, Sde_partial2, Tde_partial2);
  //cout << "Tom 明文：" << Tplaintext << endl;
  ...
  Steven.add_GT_Code(Tpair, U, Splaintext, AC1_result);   //正确
  ...
  if(bs != number - Splaintext + 1) {
    cout << "bs:" << bs << "     n-s'+1:" << number - Splaintext + 1 << endl;
  ...
  Tom.add_GT_Code(Spair, U, Tplaintext, AC2_result);
  ...
  if(Splaintext == Tplaintext) {
    cout << "The greater number is " << Splaintext << endl;
  }
